...
import static springfox.documentation.schema.Collections.collectionType;
...
/**
 * <p> 2.0 → 3.0 拓展SwaggerExpandedParameterBuilder变为拓展ExpandedParameterBuilderPlugin（框架结构终于改为接口了）
 * <p>该类作用：自定义Swagger接口文档页面，关于Get请求中的枚举参数类型，展示的数据内容：不是展示枚举名，而是描述value对应的意思
 * <p>
 * <p>从性能、合理拓展的角度来说，理想的做法是框架提供拓展，开发者自己实现插件接口，框架自动将默认的插件替换为开发者自定义的
 * <p>Post的请求体参数的枚举自定义拓展见 SwaggerDisplayConfig
 */
@Component
@SuppressWarnings("deprecation")
public class MyExpandedParameterBuilder implements ExpandedParameterBuilderPlugin {
  private final TypeResolver resolver;
  private final EnumTypeDeterminer enumTypeDeterminer;

  @Autowired
  public MyExpandedParameterBuilder(
  ...
  public void apply(ParameterExpansionContext context) {
    ...
    if (isContainerType(resolved)) {
      ...
      if (enumTypeDeterminer.isEnum(elementType.getErasedType())) {
        allowable = itemAllowables = EnumDefineUtils.allowableValues(elementType.getErasedType());
        itemTypeName = "string";
      }
              ...
              c.model(m ->
                  m.scalarModel(scalarType(elementType))
                      .facets(f -> f.enumerationFacet(
                          e -> e.allowedValues(EnumDefineUtils.allowableValues(elementType.getErasedType())))))
      ...
      modelSpecification = new ModelSpecificationBuilder()
          ...
          .facets(f -> f.enumerationFacet(
              e -> e.allowedValues(EnumDefineUtils.allowableValues(resolved.getErasedType()))
                  .build()))
          .build();
    } else {
      modelSpecification = new ModelSpecificationBuilder()
  ...
  private List<String> getEnumValues(final Class<?> subject) {
    return EnumDefineUtils.getEnumDescriptions(subject);
  }
}
